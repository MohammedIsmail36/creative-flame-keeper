...
export default function Dashboard() {
  ...
  const fetchKPIs = async () => {
    ...
    setCurrentMonthSales(
      salesItems
        .filter((i) => {
          const d = new Date(relationDate(i, "invoice", "invoice_date"));
    ...
    setSalesChange(
      chg(
        salesItems,
        (row) => relationDate(row, "invoice", "invoice_date"),
        (row) => Number(row.net_total || row.total || 0),
      ),
    );
    setPurchasesChange(
      chg(
  ...
  const fetchCharts = async () => {
    ...
    const m: MonthlyData[] = MONTH_NAMES.map((n) => ({
      name: n,
      مبيعات: 0,
      مشتريات: 0,
    }));
    const me: MonthlyExpense[] = MONTH_NAMES.map((n) => ({
      name: n,
      مصروفات: 0,
    }));
    (sR.data || []).forEach((i) => {
  ...
  const fetchLiquidity = async () => {
    ...
    rows.forEach((r: any) => {
      ...
      if (code.startsWith("1101")) cb += net;
      else if (code.startsWith("1102")) bb += net;
    });
  ...
  const fetchUnpaidInvoices = async () => {
    const { data } = await (supabase.rpc as any)("get_unpaid_invoices", {
      p_limit: 10,
    });
    if (!data) return;
    setUnpaidInvoices(
  ...
  const fetchTopProducts = async () => {
    const { data } = await (supabase.rpc as any)("get_top_products", {
      p_limit: 10,
    });
  ...
  const fetchTopCustomers = async () => {
    ...
    setTopCustomers(
      sorted.map(([id, total]) => ({
        name: id === "__cash__" ? "عميل نقدي" : nameMap.get(id) || "عميل",
        total,
