...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      setLoading(true);
      await Promise.all([fetchSummary(), fetchRecentInvoices(), fetchLowStock(), fetchMonthlyData(), fetchBalances()]);
      setLoading(false);
    };
    fetchAll();
  }, []);
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col items-end gap-2">
                  <Badge
                    variant="outline"
                    className={`text-[10px] font-semibold px-1.5 py-0 border-0 ${card.changeColor} bg-transparent`}
                  >
                    {card.change}
                  </Badge>
          ...
          <CardContent className="space-y-1 px-3">
            {recentInvoices.length === 0 ? (
              ...
              recentInvoices.slice(0, 4).map((inv) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${inv.status === "posted" ? "bg-success/10" : "bg-primary/10"}`}
                  >
                    {inv.status === "posted" ? (
            ...
            {recentInvoices.length > 0 && (
              <button
                ...
                className="w-full text-center text-xs text-primary hover:underline pt-2 pb-1 font-medium"
              >
                عرض كافة العمليات
              </button>
            )}
            ...
            <div className="space-y-2">
              {lowStockItems.map((item) => (
                ...
                >
                  ...
                  <Badge
                    variant="outline"
                    className="text-destructive border-destructive/20 bg-destructive/10 font-bold"
                  >
