...
export default function AccountBalancesReport() {
  ...
  const fetchBalances = async () => {
    ...
    if (!accountsRes.data || !linesRes.data) { setLoading(false); return; }

    const entryIds = [...new Set(linesRes.data.map((l: any) => l.journal_entry_id))];
    if (!entryIds.length) { setLoading(false); return; }
  ...
  const filtered = balances.filter(acc => {
    const matchSearch = !search || acc.name.includes(search) || acc.code.includes(search);
  ...
  balances.forEach(acc => {
    typeGroups[acc.account_type] = (typeGroups[acc.account_type] || 0) + acc.balance;
  });
    ...
    <div className="space-y-4">
        ...
        {Object.entries(typeGroups).map(([key, total]) => (
          <Card key={key} className="border-border/60 shadow-none">
            <CardContent className="p-4 text-center">
              <p className="text-xs text-muted-foreground mb-1">{typeLabels[key] || key}</p>
              <p className={`text-lg font-bold ${total >= 0 ? "text-foreground" : "text-destructive"}`}>
                {formatCurrency(Math.abs(total))}
              </p>
              <p className="text-[10px] text-muted-foreground">{total >= 0 ? "مدين" : "دائن"}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder="بحث بالاسم أو الرمز..." value={search} onChange={e => setSearch(e.target.value)} className="pr-9 h-9" />
        </div>
      ...
      <Card className="border-border/60 shadow-none">
        <CardContent className="p-0">
          {loading ? (
            <p className="text-sm text-muted-foreground text-center py-10">جاري التحميل...</p>
          ) : filtered.length === 0 ? (
              ...
              <TableBody>
                {filtered.map(acc => (
                  <TableRow key={acc.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground">{typeLabels[acc.account_type] || acc.account_type}</TableCell>
                ...
                <TableRow className="bg-muted/40 font-bold">
                  ...
                  <TableCell className={`text-sm font-bold ${(totalDebit - totalCredit) >= 0 ? "text-success" : "text-destructive"}`}>
