-- تحديث post_sales_invoice لاستخدام sales_tax_account_id من إعدادات الشركة
...
BEGIN
  ...
  SELECT locked_until_date, stock_enforcement_enabled, enable_tax, sales_tax_account_id
  ...
  -- تحديد حساب ضريبة المبيعات: من الإعدادات فقط (إذا الضريبة مفعلة وفيه قيمة ضريبة)
  IF v_tax_amount > 0 THEN
    ...
    -- تأكد أن الحساب لا يزال موجوداً
...
-- تحديث post_purchase_invoice لاستخدام purchase_tax_account_id من إعدادات الشركة
...
BEGIN
  ...
  SELECT locked_until_date, enable_tax, purchase_tax_account_id INTO v_settings FROM company_settings LIMIT 1;
